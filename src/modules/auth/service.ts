...
type AuthSuccess = { status: number; data: { token: string; refreshToken: string } }
...
export class Service {
  static async post(data: IAdmin): Promise<AuthResponse> {
    try {
      ...
      return "status" in error ? error : { status: 400, error: "Autenticación fallida" }
    }
  }
  ...
  static async refreshToken(refreshToken: string): Promise<AuthResponse> {
    try {
      ...
      const token = jwt.sign({ id: decoded.id }, process.env.JWT_SECRET || "", { expiresIn: "1d" })
